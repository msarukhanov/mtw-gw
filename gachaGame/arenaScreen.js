...
import { initGameLauncherScreen } from './gameLauncher.js'; // Переиспользуем лаунчер айфреймов!

export function getArenaScreenHTML(Game) {
    ...
    return `
        <div class="screen-content ui-element" style="${getWindowContentStyle(Game)} box-sizing: border-box; width: 100%; height: 100%; position: relative;">
...
export function initArenaScreen(container, Game, updateUiCallback) {
    ...
    container.innerHTML += getArenaScreenHTML(Game);
    ...
    container.querySelectorAll('.arena-mode-clickable').forEach(btn => {
        btn.onclick = (e) => {
            ...
            if (isLocked) {
                alert(t('arena_locked_alert', Game) || 'Вход заблокирован! Прокачайте уровень или VIP.');
                return;
            }
            ...
            initGameLauncherScreen(container, arenaTypeId, Game, updateUiCallback);
        };
    });
}
