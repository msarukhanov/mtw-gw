...
export function getGameListHTML(Game) {
    ...
    const modeTabsHTML = `
        <div style="display: flex; flex-direction: column; width: 100%; gap: 3px; margin-bottom: 12px; pointer-events: auto; flex-shrink: 0;">
            <button class="btn-game-mode" data-mode="owned" style="flex: 1; height: ${iconSize}${iconSize}; background: ${GameFilters.mode === 'owned' ? '#ffcc00' : '#222'}; color: ${GameFilters.mode === 'owned' ? '#000' : '#fff'}; border: none; font-size: calc(${iconSize} - 8px); cursor: pointer; border-radius: 4px;" title="${t('games_owned_mode', Game) || 'Мои игры'}">👤</button>
            <button class="btn-game-mode" data-mode="catalog" style="flex: 1; height: ${iconSize}; background: ${GameFilters.mode === 'catalog' ? '#ffcc00' : '#222'}; color: ${GameFilters.mode === 'catalog' ? '#000' : '#fff'}; border: none; font-size: calc(${iconSize} - 8px); cursor: pointer; border-radius: 4px;" title="${t('games_catalog_mode', Game) || 'Каталог'}">📖</button>
        </div>
    `;
    ...
    const genreFilterHTML = `
        ...
        return `<button class="btn-genre-filter" data-genre-id="${id}" style="width: ${iconSize}; height: ${iconSize}; background: ${isActive ? '#ffcc00' : '#111'}; border: 2px solid ${isActive ? '#ffcc00' : '#444'}; border-radius: 50%; padding: 0; cursor: pointer; font-size: calc(${iconSize} - 8px); flex-shrink: 0;" title="${locObj(g.title_loc, Game)}">${g.icon}</button>`;
        }).join('')}
        </div>
    `;
    ...
    const platformFilterHTML = `
        ...
        return `<button class="btn-platform-filter" data-platform-id="${id}" style="flex: 1; height: ${iconSize}; background: ${isActive ? '#ffcc00' : '#111'}; border: 1px solid ${isActive ? '#ffcc00' : '#444'}; border-radius: 4px; padding: 0; cursor: pointer; font-size: calc(${iconSize} - 8px);" title="${locObj(p.title_loc, Game)}">${p.icon}</button>`;
        }).join('')}
        </div>
    `;
    ...
    return `
        <div class="screen-content ui-element" style="${getWindowContentStyle(Game)} display: flex; flex-direction: row; box-sizing: border-box; top: 45px; height: calc(100% - 45px)">
            ${sidebarHTML}
            <div class="games-grid-container" style="${gridStyle}">
                ${displayGames.length === 0
        ? `<div style="color:#aaa; padding:20px; width:100%; text-align:center; font-size:13px;">${t('games_list_empty', Game) || 'Нет доступных игр'}</div>`
        : displayGames.map(game => getGameCardHTML(game, Game, listSettings.card_layout, displayMode)).join('')
        }
            </div>
        </div>
    `;
...
export function initGameListScreen(container, Game, updateUiCallback) {
    const refresh = () => {
        const oldScreen = container.querySelector('.screen-content');
        if (oldScreen) oldScreen.remove();
        initGameListScreen(container, Game, updateUiCallback);
    };

    // container.innerHTML += getGameListHTML(Game);

    container.insertAdjacentHTML('beforeend', getGameListHTML(Game));
    ...
    container.querySelectorAll('.game-card-clickable').forEach(card => {
        card.onclick = () => {
            ...
            initGameLauncherScreen(container, gameId, Game, updateUiCallback);
        };
    });
}
