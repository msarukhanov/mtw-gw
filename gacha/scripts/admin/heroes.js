...
function selectHero(key) {
    ...
    // Гарантируем первозданную структуру данных без изменения типов гейм-движка
    ...
    if (!h.category_ids) h.category_ids = [];
    if (!h.skills) h.skills = [];
    if (!h.extra_skills) h.extra_skills = [];
    if (!h.extra_inventory_slots) h.extra_inventory_slots = [];
    if (!h.bonds) h.bonds = [];
    if (!h.skins) h.skins = [];

    // Динамически рендерим только те статы, которые реально объявлены в механиках игры
    ...
    let baseStatsHtml = allowedStats.map(statKey => {
        ...
        return `
            <div class="form-group">
                ...
                <input type="number" value="${h.base_stats[statKey]}" oninput="target.catalog.heroes['${key}'].base_stats['${statKey}'] = parseInt(this.value) || 0">
            </div>
        `;
    }).join('');
    ...
    let growthStatsHtml = allowedStats.map(statKey => {
        ...
        return `
            <div class="form-group">
                ...
                <input type="number" value="${h.stats_growth[statKey]}" oninput="target.catalog.heroes['${key}'].stats_growth['${statKey}'] = parseInt(this.value) || 0">
            </div>
        `;
    }).join('');

    // Подготовка выпадающих опций (Справочники)
    ...
    const elementOptions = Object.keys(target.catalog.hero_elements || {}).map(elKey =>
        `<option value="${elKey}" ${h.element_id === elKey ? 'selected' : ''}>${target.catalog.hero_elements[elKey].icon || ''} ${target.catalog.hero_elements[elKey].title_loc?.en || elKey}</option>`
    ).join('');
    ...
    ed.innerHTML = `
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <div class="sub-section" style="padding:10px;">${generateLocInputs(h.title_loc, `target.catalog.heroes['${key}'].title_loc`)}</div>
            </div>
            <div class="form-group">
                <label>Hero Biography (Localization)</label>
                <div class="sub-section" style="padding:10px;">${generateLocInputs(h.bio_loc, `target.catalog.heroes['${key}'].bio_loc`)}</div>
            </div>
        </div>
        <div id="hero-form-divider-anchor"></div>
    `;
    // Продолжение сборки формы полей, списков тегов и связей будет в следующих частях
    // Продолжение сборки формы из Части 1
    const formDivider = document.getElementById('hero-form-divider-anchor');
    if (formDivider) {
        formDivider.outerHTML = `
            <div class="form-grid" style="margin-top: 15px">
                ...
                <div class="form-group"><label>Max Level Cap</label><input type="number" value="${h.max_level || 100}" oninput="target.catalog.heroes['${key}'].max_level = parseInt(this.value) || 0"></div>
                ...
                <div class="form-group">
                    ...
                    <select onchange="target.catalog.heroes['${key}'].element_id = this.value">
            ...
            <!-- УПРАВЛЕНИЕ МАССИВОМ КАТЕГОРИЙ (category_ids) -->
            <div class="sub-section" style="margin-top:15px; border-color: var(--accent-blue);">
                <div class="sub-section-title" style="color:var(--accent-blue);">🏷️ Hero Categories / Tags</div>
                <div style="margin-bottom:8px; display:flex; flex-wrap:wrap;">
                    ${h.category_ids.map((catId, cIdx) => `
                        <span class="badge" style="display:inline-flex; align-items:center; gap:5px; margin:2px; padding:4px 8px; background:var(--accent-blue);">
                            ${target.catalog.hero_categories[catId]?.title_loc?.en || catId}
                            <b style="cursor:pointer; color:#ff3333;" onclick="target.catalog.heroes['${key}'].category_ids.splice(${cIdx},1); selectHero('${key}');">×</b>
                        </span>
                    `).join('') || '<span style="font-size:11px; color:var(--text-muted); padding:2px;">No categories assigned</span>'}
                </div>
                <div style="display:flex; gap:6px;">
                    <select id="add-hero-cat-${key}" style="font-size:12px; padding:4px;">
                        <option value="">-- Bind Category Tag --</option>
                        ${Object.keys(target.catalog.hero_categories || {}).filter(c => !h.category_ids.includes(c)).map(c => `<option value="${c}">${target.catalog.hero_categories[c].title_loc?.en || c}</option>`).join('')}
                    </select>
                    <button class="primary" style="padding:4px 10px; font-size:12px;" onclick="const val=document.getElementById('add-hero-cat-${key}').value; if(val){target.catalog.heroes['${key}'].category_ids.push(val); selectHero('${key}');}">+ Bind</button>
                </div>
            </div>

            <!-- УПРАВЛЕНИЕ АКТИВНЫМИ И ПАССИВНЫМИ НАВЫКАМИ (skills & extra_skills) -->
            <div class="sub-section" style="margin-top:15px;">
                <div class="sub-section-title">⚔️ Combat Active Skills Portfolio</div>
                <div style="margin-bottom:8px; display:flex; flex-wrap:wrap;">
                    ${h.skills.map((sKey, sIdx) => `
                        <span class="badge" style="display:inline-flex; align-items:center; gap:5px; margin:2px; padding:4px 8px; background:var(--accent-pink);">
                            ${target.catalog.skills[sKey]?.icon || '👑'} ${target.catalog.skills[sKey]?.title_loc?.en || sKey}
                            <b style="cursor:pointer; color:#ff3333;" onclick="target.catalog.heroes['${key}'].skills.splice(${sIdx},1); selectHero('${key}');">×</b>
                        </span>
                    `).join('') || '<span style="font-size:11px; color:var(--text-muted); padding:2px;">No active skills assigned</span>'}
                </div>
                <div style="display:flex; gap:6px;">
                    <select id="add-hero-skill-${key}" style="font-size:12px; padding:4px;">
                        <option value="">-- Learn Skill Slot --</option>
                        ${Object.keys(target.catalog.skills || {}).filter(s => !h.skills.includes(s)).map(s => `<option value="${s}">${target.catalog.skills[s].icon || ''} ${target.catalog.skills[s].title_loc?.en || s}</option>`).join('')}
                    </select>
                    <button class="primary" style="padding:4px 10px; font-size:12px;" onclick="const val=document.getElementById('add-hero-skill-${key}').value; if(val){target.catalog.heroes['${key}'].skills.push(val); selectHero('${key}');}">+ Learn</button>
                </div>
            </div>

            <!-- УПРАВЛЕНИЕ ДОПОЛНИТЕЛЬНЫМИ СЛОТАМИ ИНВЕНТАРЯ (extra_inventory_slots) -->
            <div class="sub-section" style="margin-top:15px; border-color: var(--border-color);">
                <div class="sub-section-title">🎒 Extra Inventory Slots Specs</div>
                <div style="margin-bottom:8px; display:flex; flex-wrap:wrap;">
                    ${h.extra_inventory_slots.map((slot, sIdx) => `
                        <span class="badge" style="display:inline-flex; align-items:center; gap:5px; margin:2px; padding:4px 8px; background:#4a4a4a;">
                            ${slot.toUpperCase()}
                            <b style="cursor:pointer; color:#ff3333;" onclick="target.catalog.heroes['${key}'].extra_inventory_slots.splice(${sIdx},1); selectHero('${key}');">×</b>
                        </span>
                    `).join('') || '<span style="font-size:11px; color:var(--text-muted); padding:2px;">No extra slots unlocked</span>'}
                </div>
                <div style="display:flex; gap:6px;">
                    <input type="text" id="add-hero-slot-${key}" placeholder="e.g. amulet, relic" style="font-size:12px; padding:4px 8px;">
                    <button class="primary" style="padding:4px 10px; font-size:12px;" onclick="const val=document.getElementById('add-hero-slot-${key}').value.trim().toLowerCase(); if(val && !target.catalog.heroes['${key}'].extra_inventory_slots.includes(val)){target.catalog.heroes['${key}'].extra_inventory_slots.push(val); selectHero('${key}');}">+ Unlock Slot</button>
                </div>
            </div>
            
            <div id="hero-bonds-skins-anchor"></div>
        `;
    }
    // Продолжение сборки формы из Части 2
    const bondsSkinsGroup = document.getElementById('hero-bonds-skins-anchor');
    if (bondsSkinsGroup) {
        // Опции для выбора компаньона в связях (исключая текущего героя)
                ...
                <div class="sub-section" style="margin-top:10px; padding:12px; border-color:var(--accent-blue);">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" value="${bond.bonus_value || 0}" oninput="target.catalog.heroes['${key}'].bonds[${bIdx}].bonus_value = parseInt(this.value) || 0">
                        </div>
                        <div class="form-group">
...
function createNewHero() {
    if (!target.catalog.heroes) target.catalog.heroes = {};
    const newKey = `new_deity_${Object.keys(target.catalog.heroes).length}`;

    // Глубокое копирование прототипа из констант твоего движка
    ...
    target.catalog.heroes[newKey].base_stats = { hp: 1000, atk: 100, speed: 100 };
    target.catalog.heroes[newKey].stats_growth = { hp: 50, atk: 10, speed: 2 };

    selectHero(newKey);
}

function deleteHero(key) {
    if (!confirm('Are you sure you want to permanently delete this hero instance?')) return;
...
function renameHeroKey(oldKey, newKey) {
    ...
    if (isKeyDuplicate('hero', newKey, oldKey)) {
        alert(`Error: Hero key "${newKey}" already exists in data roster!`);
        renderHeroes();
        selectHero(oldKey);
        return;
    }
