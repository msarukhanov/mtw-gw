...
export function getHeroListHTML(Game) {
    ...
    const factionFilterHTML = `
        ...
        return `<button class="btn-faction-filter" data-faction-id="${id}" style="width: ${iconSize}; height: ${iconSize}; background: ${isActive ? '#ffcc00' : '#111'}; border: 2px solid ${isActive ? '#ffcc00' : '#444'}; border-radius: 50%; padding: 0; cursor: pointer; font-size: calc(${iconSize} - 8px); flex-shrink: 0;" title="${locObj(f.title_loc, Game)}">${f.icon}</button>`;
    }).join('')}
        </div>
    `;
    ...
    const classFilterHTML = `
        ...
        return `<button class="btn-class-filter" data-class-id="${id}" style="flex: 1; height: ${iconSize}; background: ${isActive ? '#ffcc00' : '#111'}; border: 1px solid ${isActive ? '#ffcc00' : '#444'}; border-radius: 4px; padding: 0; cursor: pointer; font-size: calc(${iconSize} - 8px);" title="${locObj(c.title_loc, Game)}">${c.icon}</button>`;
    }).join('')}
        </div>
    `;
    ...
    if (ListFilters.mode === 'owned') {
        ...
        displayHeroes.sort((a, b) => {
            const powerA = getHeroRating(a, Game);
            const powerB = getHeroRating(b, Game);
    ...
    return `
        ...
        ? `<div style="color:#aaa; padding:20px; width:100%; text-align:center; font-size:13px;">${t('inventory_empty', Game)}</div>`
        : displayHeroes.map(hero => getHeroCardHTML(hero, Game, listSettings.card_layout, displayMode)).join('')
        }
            </div>
        </div>
    `;
...
export function initHeroListScreen(container, Game, updateUiCallback) {
    const refresh = () => {
        const oldScreen = container.querySelector('.screen-content');
        if (oldScreen) oldScreen.remove();
        initHeroListScreen(container, Game, updateUiCallback);
    };

    // container.innerHTML += getHeroListHTML(Game);

    container.insertAdjacentHTML('beforeend', getHeroListHTML(Game));

    DialogManager.trigger('OPEN_HEROES_FIRST_TIME', Game);
    ...
    container.querySelectorAll('.hero-card-clickable').forEach(card => {
        card.onclick = () => {
            ...
            initHeroViewScreen(container, instanceId, currentListIds, Game, updateUiCallback);
        };
    });
}